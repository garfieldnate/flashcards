...
export default class SM2Plus {
  ...
  const getVarsForCard = (cardData, dateNow) => {
    var sm2pVars = self.model.getSM2PVars(cardData);
    return sm2pVars || self.getDefaultVariables(dateNow);
  }

  const updateSm2p = (cardData, performanceRating) => {
    const dateNow = self.dateProvider.now();
    const sm2pVars = self.getVarsForCard(cardData, dateNow);
    const correct = performanceRating >= correctThreshold;
    const percentOverdue = correct ?
      min(2, self.getPercentOverdue(dateNow, sm2pVars)) :
      1;
    // TODO: ensure this gets written to the model
    sm2pVars.difficulty += percentOverdue * 1 / 17 * (8 - 9 * performanceRating)
    difficultyWeight = self.getDifficultyWeight(sm2pVars['difficulty'])
  ...
  const getPercentOverdue = (dateNow, sm2pVars) => {
    const delta = dateNow - self.dateProvider.fromUnix(sm2pVars.dateLastReviewed);
  ...
  */
  const prioritizeStudy = (cardList) => {
    dateNow = self.dateProvider.now()
    prioritizedCardList = []
    cardList.forEach((cardData) => {
      sm2pVars = self.model.getVarsForCard(cardData)
      cardData.percentOverdue = self.getPercentOverdue(dateNow, sm2pVars)
